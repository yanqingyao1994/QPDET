...
from mmdet.ops import DeformConv, arb_batched_nms
...
class AOGModule(nn.Module):
    #adaptive oriented anchor generate
    ...
    def get_anchor_single(self, points, shape_pred, theta_pred, base_len):
        ...
        theta_pred = theta_pred.permute(0, 2, 3, 1).reshape(-1, 1)
        theta_pred = theta_pred * self.theta_std

        wh = shape_pred[:, :2] + shape_pred[:, 2:]
        center = points[:, :2] + shape_pred[:, 2:] - shape_pred[:, :2]
        anchors = torch.cat([center, wh, theta_pred], dim=-1)
...
class AOGRPNHead(OBBAnchorHead, RPNTestMixin):
    ...
    def __init__(self,
        ...
        self.in_channels = in_channels
        self.feat_channels = feat_channels
        self.adaptive_oanchor_generator = AOGModule(
            in_channels,
            feat_channels,
            strides,
            base_edge_list,
    ...
    def init_weights(self):
        self.adaptive_oanchor_generator.init_weights()
    ...
    def forward_single(self, x, inds):
        shape_pred, theta_pred, centerness, anchors, offsets = \
                self.adaptive_oanchor_generator.forward_single(x, inds)
    ...
    def loss(self,
        ...
        loss = dict()
        loss.update(self.adaptive_oanchor_generator.loss(
            shape_preds, theta_preds, centernesses, gt_bboxes))
    ...
    def _get_bboxes_single(self,
        ...
        # scores = torch.sigmoid(rpn_centerness) #############################
